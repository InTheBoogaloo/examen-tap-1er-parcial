 ...
 * @param array $opts   Opciones: upper, lower, digits, symbols,
 *                      avoid_ambiguous, exclude, require_each.
 * @return string
...
function generate_password(int $length = 16, array $opts = []): string {
    ...
    if ($pool === '') {
        throw new InvalidArgumentException(
            'No hay caracteres disponibles para generar la contraseña (pool vacío).'
        );
    }
 ...
 *
 * @param int   $count  Número de contraseñas (>= 1).
...
function generate_passwords(int $count = 5, int $length = 16, array $opts = []): array {
    if ($count < 1) {
        throw new InvalidArgumentException('El count debe ser >= 1');
    }
